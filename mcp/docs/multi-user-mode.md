...
The Penjar MCP server supports a multi-user mode, allowing multiple Penjar users
...
To run the MCP server and the Penjar MCP plugin in multi-user mode (for testing),
...
* build and launch the Penjar MCP plugin server in multi-user mode.

See the package.json scripts for both `mcp-server` and `penjar-plugin` for details.
...
* The same token must be provided when connecting the Penjar MCP plugin
  to the MCP server.    
  In the future, the token will, most likely be generated by Penjar and
