...
| Desktop shell/runtime | `frontend/src/app/main/ui.cljs`, `frontend/src/app/main/ui/routes.cljs`, `frontend/src/app/main/router.cljs` | Browser-first shell, route composition, and store-driven navigation live in ClojureScript UI runtime | Flutter shell with native window lifecycle + route/state bridge to shared contracts | Core Flutter shell is established; runtime-switchable in-memory/remote-stub contract routing, mode parity gate, operation-scoped remote-stub blocked-operation profile, and scripted transport-client injection path are implemented, but route/state integration and real backend transport implementation are pending | Desktop Flutter Program | 2026-05-15 | In progress | [Web + Desktop Phase C Execution Log](/technical-guide/developer/web-mcp-phase-c-execution-log/) |
