...
14. Auth backend failure-taxonomy fallback status mapping baseline:
   ...
   - explicit failure-flag alias handling now includes additional alias keys (`isOk`, `is_success`, `is_ok`) for signed-out inference and fallback status mapping compatibility, including mixed-alias explicit-false precedence locks (`success=true` + `is_success=false`),
