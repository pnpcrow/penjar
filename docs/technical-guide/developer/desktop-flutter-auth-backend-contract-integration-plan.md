...
14. Auth backend failure-taxonomy fallback status mapping baseline:
   ...
   - backend code extraction now includes additional status aliases (`statusCode`, `httpStatus`, `status_code`, `http_status`) for signed-out/session-expired inference compatibility,
   - explicit signed-in aliases remain precedence over signed-out code variants, with regression coverage now including `SESSION_TIMEOUT` / `EXPIRED_TOKEN` variant cases,
