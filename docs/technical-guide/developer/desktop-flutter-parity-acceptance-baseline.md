...
- Auth-session parity suite now includes backend integration flow coverage in `desktop/test/parity/auth_session_parity_test.dart` (backend snapshot sign-in/signed-out transition + required-state fallback blocking + code-only backend failure status mapping + statusCode/status_code unauthorized mapping + snake-case/isOk failure-flag auth-failed mapping + snake_case + logged-style camelCase signed-in/remember alias normalization including dedicated `loggedIn` + `isLoggedIn` locks + full signed-out alias variant fallback-status mapping (`signedOut`, `isSignedOut`, `loggedOut`, `isLoggedOut`, `signed_out`, `is_signed_out`, `logged_out`, `is_logged_out`) + session-timeout session-expired mapping + payload-envelope wrapper parity coverage for `payload` and nested `result -> payload` paths).
- Auth backend contract fixture matrix baseline is now maintained in `desktop/test/contracts/workflow_contracts_test.dart` (`auth backend contract fixture: ...` cases), including `result/data/payload` envelope-chain + `authState` alias coverage, status-code alias extraction coverage (`statusCode` / `httpStatus` / `status_code` / `http_status`), explicit signed-out envelope alias fixture coverage (`signedOut`, `is_signed_out`), explicit logged-style envelope fixtures (`data.authState.loggedIn`, `result.authState.loggedOut`, `data.authState.is_logged_out`), logged-style signed-out alias regression coverage (`loggedOut`, `isLoggedOut`, `logged_out`, `is_logged_out`), and explicit signed-in precedence regression cases for `SESSION_TIMEOUT` / `EXPIRED_TOKEN`, statusCode/status_code unauthorized variants, snake-case/isOk failure-flag variants, and explicit signedIn vs signedOut/isSignedOut/loggedOut/isLoggedOut/signed_out/is_signed_out/logged_out/is_logged_out state-alias collisions, to lock normalized signed-in/status precedence behavior before real backend auth contract binding.
