...
- Windows installer flow now includes protocol registration command-hook baseline (`PENJAR_WINDOWS_PROTOCOL_REGISTER_COMMAND`) with optional `PENJAR_WINDOWS_PROTOCOL_SCHEME` / `PENJAR_WINDOWS_PROTOCOL_TARGET_PATH` overrides and strict gate control (`STRICT_WINDOWS_PROTOCOL_REGISTRATION`); real production registry/installer command provisioning remains pending.
