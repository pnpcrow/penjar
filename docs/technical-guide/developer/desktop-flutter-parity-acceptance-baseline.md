...
- Auth backend contract fixture matrix baseline is now maintained in `desktop/test/contracts/workflow_contracts_test.dart` (`auth backend contract fixture: ...` cases), including `result/data` envelope + `authState` alias coverage, status-code alias extraction coverage (`statusCode` / `httpStatus` / `status_code` / `http_status`), explicit signed-out envelope alias fixture coverage (`signedOut`, `is_signed_out`), and explicit signed-in precedence regression cases for `SESSION_TIMEOUT` / `EXPIRED_TOKEN`, statusCode/status_code unauthorized variants, snake-case/isOk failure-flag variants, and explicit signedIn vs signedOut/isSignedOut/signed_out/is_signed_out state-alias collisions, to lock normalized signed-in/status precedence behavior before real backend auth contract binding.
