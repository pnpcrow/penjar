...
- Auth parser now treats explicit backend failure flags (`success`, `ok`, `isSuccess`) set to false as signed-out inference precedence when explicit signed-in aliases are absent.
