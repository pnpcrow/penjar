...
- Auth contract now supports strict malformed-backend-auth schema fallback gate (`RemoteStubAuthSessionContract.strictBackendSchema`) that blocks delegate fallback when backend payload is present but does not satisfy auth-state parser schema.
...
- Runtime remote profile summary now surfaces strict auth backend schema mode as `auth-backend-schema: strict` for diagnostics visibility.
