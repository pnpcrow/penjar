...
- Remote-stub auth backend response parsing now normalizes alias payloads for remember/signed-in fields (`remember`, `persistSession`, `isAuthenticated`, `authenticated`) and token/session/user hints (`accessToken`, `token`, `sessionToken`, `refreshToken`, `sessionId`, `user`) with explicit signed-out override precedence.
