...
- Auth-session parity suite now includes backend integration flow coverage in `desktop/test/parity/auth_session_parity_test.dart` (backend snapshot sign-in/signed-out transition + required-state fallback blocking + code-only backend failure status mapping + statusCode unauthorized mapping + session-timeout session-expired mapping).
- Auth backend contract fixture matrix baseline is now maintained in `desktop/test/contracts/workflow_contracts_test.dart` (`auth backend contract fixture: ...` cases), including `result/data` envelope + `authState` alias coverage, status-code alias extraction coverage (`statusCode` / `httpStatus`), and explicit signed-in precedence regression cases for `SESSION_TIMEOUT` / `EXPIRED_TOKEN` and `statusCode` unauthorized variants, to lock normalized signed-in/status precedence behavior before real backend auth contract binding.
