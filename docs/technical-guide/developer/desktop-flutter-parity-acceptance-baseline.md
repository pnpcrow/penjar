...
- Auth-session parity suite now includes backend integration flow coverage in `desktop/test/parity/auth_session_parity_test.dart` (backend snapshot sign-in/signed-out transition + required-state fallback blocking + code-only backend failure status mapping + statusCode/status_code unauthorized mapping + snake-case/isOk failure-flag auth-failed mapping + snake_case signed-in/remember alias normalization + full signed-out alias variant fallback-status mapping (`signedOut`, `isSignedOut`, `loggedOut`, `isLoggedOut`, `signed_out`, `is_signed_out`, `logged_out`, `is_logged_out`) + session-timeout session-expired mapping + payload-envelope wrapper parity coverage for `payload` and nested `result -> payload` paths).
...
- Additional auth contract regressions now include snake_case signed-in/remember alias normalization, full signed-out alias variant fallback-status mapping, and `signed_in` override precedence under unauthorized code payloads.
