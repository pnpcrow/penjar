...
|---|---|---|---|---|---|
| Authentication/session | Sign in, session restore, token refresh | In progress | Auth + Desktop Integration | [Desktop Flutter Migration Inventory](/technical-guide/developer/desktop-flutter-migration-inventory/) | Flutter auth/session parity scaffold, parity test harness, runtime-switchable in-memory/remote-stub contract boundary, backend request metadata mapping, backend execution-path error propagation, backend response-driven state mutation wiring, auth snapshot store/seed seam, file-backed snapshot path, command-hook secure-store bridge path, flutter_secure_storage-backed native credential-store adapter path, strict/mirror rollout controls, default-on secure-store rollout telemetry labels, legacy-retirement strict enforcement control, and legacy decommission guard automation are added; real backend contract integration and command/file path hard removal are pending. |
