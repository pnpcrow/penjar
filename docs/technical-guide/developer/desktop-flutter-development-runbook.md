...
Latest auth evidence lock: `Unit WS-D-283` optimizes exact-length candidate reuse in
`_classifyBackendCode(...)` by reusing `trimmedExactLengthCandidate` when
`compactLength == trimmedLength`, and locks capitalized compact marker-length-candidate near-miss
...
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-283`).
