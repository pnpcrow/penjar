...
Latest release pipeline evidence lock: `Unit WS-D-291` expands Windows installer pipeline contract
coverage with strict installer + protocol command-failure interaction locks across release/debug
build modes, ensuring strict installer mode also blocks on protocol non-zero execution failures;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-291`).
