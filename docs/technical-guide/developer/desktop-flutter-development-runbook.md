...
Latest auth evidence lock: `Unit WS-D-254` unifies exact-marker classification through
`_backendExactCodeClassifications` single-map lookup in `_classifyBackendCode(...)` and locks
compact `unauthorized` fallback behavior in contract/parity suites while preserving
numeric/marker-length/substring fallback semantics; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-254`).
