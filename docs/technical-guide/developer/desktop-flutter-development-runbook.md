...
Latest release pipeline evidence lock: `Unit WS-D-289` expands Windows installer pipeline contract
coverage with strict missing-runner failure locks across release/debug build modes for both
installer strict mode and strict protocol registration mode; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-289`).
