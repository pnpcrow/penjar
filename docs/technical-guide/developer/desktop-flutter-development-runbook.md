...
Latest release pipeline evidence lock: `Unit WS-D-286` wires Windows installer pipeline contract
checks into both desktop release workflows (`release-desktop-installer-smoke.yml` signing-readiness
execution + `tests-desktop-flutter.yml` verify-matrix artifact upload), extending strict protocol
registration regression visibility beyond local verify to CI artifact trails; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-286`).
