...
Latest release pipeline evidence lock: `Unit WS-D-296` expands Windows installer pipeline contract
coverage with strict/strict-protocol mode alias parsing locks (`true|yes|strict`) so strict-mode
behavior remains deterministic across accepted toggle aliases; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-296`).
