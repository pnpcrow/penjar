...
Latest release pipeline evidence lock: `Unit WS-D-322` extends strict numeric near-match regression
coverage to signed/exponent numeric aliases (`"+1"`, `"1e0"`) in installer/protocol paths,
locking exact numeric strict alias boundary (`"1"` only) behavior; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-322`).
