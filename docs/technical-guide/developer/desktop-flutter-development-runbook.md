...
Latest release pipeline evidence lock: `Unit WS-D-304` optimizes strict toggle parsing by
replacing per-toggle external lowercase command execution with builtin pattern matching while
preserving trim-normalized strict semantics; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-304`).
