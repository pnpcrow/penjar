...
Latest release pipeline evidence lock: `Unit WS-D-328` extends strict numeric near-match regression
coverage to trim-wrapped numeric aliases (`" 1.0 "`, `"\\t+1\\t"`) in installer/protocol paths,
locking exact numeric strict alias boundary (`"1"` only) behavior; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-328`).
