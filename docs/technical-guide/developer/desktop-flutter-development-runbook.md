...
Latest auth evidence lock: `Unit WS-D-277` optimizes marker-threshold guard access in
`_classifyBackendCode(...)` by caching marker minimum-length thresholds as local values before
threshold checks to reduce repeated top-level lookups on classifier hot paths, and locks
capitalized compact `code: "SignedOut"` deterministic auth-required fallback behavior in
