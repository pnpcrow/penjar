...
Latest auth evidence lock: `Unit WS-D-249` adds exact-marker set fast paths in
`_classifyBackendCode(...)` so common compact backend codes classify without substring loops while
preserving fallback semantics for composite code payloads; execution evidence is recorded in
