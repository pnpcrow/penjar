...
Latest auth evidence lock: `Unit WS-D-250` adds minimum-marker-length short-circuits in
`_classifyBackendCode(...)` so compact inputs that cannot contain any signed-out/session-expired
marker bypass substring loops while preserving exact-marker and composite fallback semantics;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-250`).
