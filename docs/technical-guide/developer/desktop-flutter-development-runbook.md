...
Latest auth evidence lock: `Unit WS-D-274` optimizes marker scan loops in
`_classifyBackendCode(...)` by using index-based iteration with cached marker counts and immediate
session-expired marker return to reduce iterator overhead on classifier hot paths, and locks
capitalized compact `code: "Unauthenticated"` deterministic auth-required fallback behavior in
contract/parity suites; execution evidence is recorded in `web-mcp-phase-c-execution-log.md`
(`Unit WS-D-274`).
