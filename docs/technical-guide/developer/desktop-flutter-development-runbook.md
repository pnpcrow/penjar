...
Latest release pipeline evidence lock: `Unit WS-D-294` fixes strict-protocol gating so installer
command non-zero failures no longer hard-fail strict protocol mode when protocol registration
succeeds, and locks that behavior in release/debug contract cases; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-294`).
