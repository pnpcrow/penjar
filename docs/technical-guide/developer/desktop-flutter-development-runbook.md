...
Latest auth evidence lock: `Unit WS-D-245` consolidates backend code signed-out/session-expired
classification into a single auth parser path in
`desktop/lib/contracts/remote_stub_contracts.dart`, preserving existing signed-out precedence while
removing duplicate backend-code compact/marker scans in auth-state inference; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-245`).
