...
Latest auth evidence lock: `Unit WS-D-234` completed sign-in direct-wrapper explicit signed-in-false
alias symmetry (`isAuthenticated` / `loggedIn` / `isLoggedIn` / `is_authenticated` /
`signedIn` / `authenticated` / `signed_in` / `is_signed_in` / `logged_in` /
`is_logged_in`) across contract+parity suites; see
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-234`).
