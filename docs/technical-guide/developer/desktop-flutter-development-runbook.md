...
Latest auth evidence lock: `Unit WS-D-236` completed restore-session direct `result.authState`
signed-out alias fixture closure (`signedOut` / `loggedOut`) to align result/data wrapper
coverage symmetry for deterministic fallback mapping; see
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-236`).
