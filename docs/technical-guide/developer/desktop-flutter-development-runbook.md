...
Latest release pipeline evidence lock: `Unit WS-D-319` hardens
`check_windows_installer_pipeline_contract.sh` with duplicate case-name preflight validation,
failing fast when matrix case IDs collide and preserving deterministic report/readability as the
suite grows; execution evidence is recorded in `web-mcp-phase-c-execution-log.md`
(`Unit WS-D-319`).
