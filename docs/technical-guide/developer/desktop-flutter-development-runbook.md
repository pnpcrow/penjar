...
Latest release pipeline evidence lock: `Unit WS-D-305` extends strict toggle trim regression
coverage to whitespace-wrapped numeric strict aliases (`" 1 "`), locking strict
installer/protocol fail semantics for numeric-path normalization; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-305`).
