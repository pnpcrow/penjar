...
Latest auth evidence lock: `Unit WS-D-251` removes duplicate trim work in auth backend code
normalization by compacting from classifier-local trimmed input
(`_compactBackendCodeFromTrimmed(trimmed)`), preserving existing numeric/exact/substring fallback
semantics; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-251`).
