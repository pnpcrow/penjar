...
Latest auth evidence lock: `Unit WS-D-261` consolidates raw exact-code fast-path classification in
`_classifyBackendCode(...)` to single-map lookup
(`_backendRawExactCodeClassifications`) for lowercase/uppercase compact exact markers, and locks
uppercase compact `SESSIONEXPIRED` session-expired fallback behavior in contract/parity suites;
