...
Latest auth evidence lock: `Unit WS-D-275` optimizes compact-length guards in
`_classifyBackendCode(...)` by caching `compactLength` for compact-empty and marker-min-length
checks to reduce repeated length getter lookups on classifier hot paths, and locks capitalized
compact `code: "LoggedOut"` deterministic auth-required fallback behavior in contract/parity
suites; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-275`).
