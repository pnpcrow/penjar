...
Latest auth evidence lock: `Unit WS-D-271` optimizes compact normalization hot-loop behavior in
`_compactBackendCodeFromTrimmed(...)` by replacing local lazy-buffer closure dispatch with inline
`asciiBuffer ??= StringBuffer()` initialization while preserving lazy allocation semantics, and
locks unicode-only `code: "\uC138\uC158\uB9CC\uB8CC"` signed-in/status stability behavior without
unintended auth-required/session-expired fallback mapping in contract/parity suites; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-271`).
