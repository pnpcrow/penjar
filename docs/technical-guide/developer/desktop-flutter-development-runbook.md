...
Latest release pipeline evidence lock: `Unit WS-D-295` expands strict protocol regression coverage
for installer placeholder interactions, locking release/debug non-blocking warning behavior when
protocol registration succeeds; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-295`).
