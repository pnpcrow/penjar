...
Latest auth evidence lock: `Unit WS-D-257` adds non-compact ASCII code-unit normalization in
`_compactBackendCodeFromTrimmed(...)` to avoid full-string `toLowerCase` allocation on
delimiter-heavy ASCII code inputs while preserving non-ASCII fallback behavior, and locks
delimited uppercase `UNAUTHORIZED::TOKEN` auth-required fallback behavior in contract/parity
suites; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-257`).
