...
Latest release pipeline evidence lock: `Unit WS-D-301` hardens strict toggle parsing by trimming
leading/trailing whitespace before case-insensitive normalization, with contract coverage for
whitespace aliases (for example `" yes "`, `" true "`) across strict installer/protocol
paths; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-301`).
