...
Latest release pipeline evidence lock: `Unit WS-D-300` locks case-insensitive strict toggle alias
coverage (`YES`, `TRUE`, `StRiCt`, `YeS`) in Windows installer pipeline contract checks so
strict/strict-protocol parsing remains deterministic across release/debug paths; execution evidence
is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-300`).
