...
Latest auth evidence lock: `Unit WS-D-268` optimizes compact no-delimiter uppercase-tail
normalization in `_compactBackendCodeFromTrimmed(...)` by tracking
`firstUppercaseCompactIndex` and lowercasing only from that index onward, and locks
lowercase-prefix uppercase-suffix `unauthorizedTOKEN` auth-required fallback behavior in
