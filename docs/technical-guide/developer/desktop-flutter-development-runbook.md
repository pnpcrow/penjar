...
Latest release pipeline evidence lock: `Unit WS-D-318` extends strict near-match regression
coverage to placeholder warning paths, locking non-strict warning semantics for near-match aliases
(`truee`) in installer/protocol command-placeholder scenarios; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-318`).
