...
Latest auth evidence lock: `Unit WS-D-265` optimizes delimiter-path Unicode-suffix normalization
in `_compactBackendCodeFromTrimmed(...)` by appending Unicode-lowercased compact suffix units
from the first non-ASCII index without rescanning already-normalized ASCII prefix segments, and
locks mixed-case delimited `Unauthorized::TOKEN토큰` auth-required fallback behavior in
contract/parity suites; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-265`).
