...
Latest auth evidence lock: `Unit WS-D-255` reorders substring classification to scan
session-expired markers first and signed-out-only markers second (single session-marker pass) in
`_classifyBackendCode(...)`, and locks mixed
`UNAUTHORIZED_REFRESH_TOKEN_EXPIRED` session-expired fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-255`).
