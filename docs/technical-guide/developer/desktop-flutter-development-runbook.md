...
Latest auth evidence lock: `Unit WS-D-267` optimizes delimiter-path scan progression in
`_compactBackendCodeFromTrimmed(...)` by skipping recheck of the already-identified first
non-compact delimiter index and continuing from `firstNonCompactIndex + 1`, and locks lowercase
trailing-delimiter `unauthorized::` auth-required fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-267`).
