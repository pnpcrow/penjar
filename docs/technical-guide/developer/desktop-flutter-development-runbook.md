...
Latest auth evidence lock: `Unit WS-D-264` optimizes delimiter-path compact normalization in
`_compactBackendCodeFromTrimmed(...)` by reusing pre-scanned compact prefix segments and
continuing normalization from `firstNonCompactIndex`, and locks mixed-case delimited
`Unauthorized::TOKEN` auth-required fallback behavior in contract/parity suites; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-264`).
