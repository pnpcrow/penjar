...
Latest auth evidence lock: `Unit WS-D-258` removes separate non-ASCII pre-scan in
`_compactBackendCodeFromTrimmed(...)` by using single-pass ASCII normalization with inline
non-ASCII fallback to Unicode normalization helper, and locks delimited uppercase
`UNAUTHORIZED::토큰` auth-required fallback behavior in contract/parity suites; execution evidence
is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-258`).
