...
Latest release pipeline evidence lock: `Unit WS-D-290` expands Windows installer pipeline contract
coverage with strict command-execution failure locks (installer/protocol) across release/debug
build modes, ensuring deterministic strict failure behavior for non-zero command exits; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-290`).
