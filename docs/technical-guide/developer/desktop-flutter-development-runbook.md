...
Latest auth evidence lock: `Unit WS-D-237` completed auth fixture matrix symmetry automation
for direct `authState` aliases (`sign-in` / `refresh-token` / `restore-session` × `result/data`
wrappers × signed-in/signed-out alias sets), with execution evidence in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-237`).
