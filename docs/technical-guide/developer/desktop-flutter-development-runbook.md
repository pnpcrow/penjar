...
Latest auth evidence lock: `Unit WS-D-247` extends signed-out/session-expired code classification
coverage to include `JWT_EXPIRED` normalization (`jwtexpired`) and adds contract/parity regression
locks for both fallback mapping and explicit signed-in override precedence; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-247`).
