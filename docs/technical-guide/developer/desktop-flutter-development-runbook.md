...
Latest release pipeline evidence lock: `Unit WS-D-342` optimizes installer-pipeline contract
assertion scanning by using fixed-string file checks (`grep -Fq`) instead of full-file shell-string
loading for report/log pattern verification, reducing per-case allocation overhead while preserving
existing assertion semantics; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-342`).
