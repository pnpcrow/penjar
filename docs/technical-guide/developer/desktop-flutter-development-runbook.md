...
Latest auth evidence lock: `Unit WS-D-256` adds an uppercase-compact backend-code fast path in
`_compactBackendCodeFromTrimmed(...)` (direct lowercase return for fully ASCII alphanumeric
uppercase inputs without delimiter-filter buffer loops), and locks uppercase compact
`UNAUTHORIZED` auth-required fallback behavior in contract/parity suites; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-256`).
