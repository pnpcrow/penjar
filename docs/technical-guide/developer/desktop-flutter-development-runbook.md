...
Latest auth evidence lock: `Unit WS-D-259` adds uppercase exact-code fast-path classification in
`_classifyBackendCode(...)` via `_backendUppercaseExactCodeClassifications`, and locks uppercase
compact `TOKENEXPIRED` session-expired fallback behavior in contract/parity suites; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-259`).
