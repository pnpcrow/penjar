...
Latest release pipeline evidence lock: `Unit WS-D-303` extends strict toggle trim regression
coverage to newline-wrapped aliases (for example `"\nYES\n"`, `"\nTRUE\n"`) so whitespace-class
normalization remains deterministic for strict installer/protocol parsing; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-303`).
