...
Latest release pipeline evidence lock: `Unit WS-D-323` extends strict numeric near-match regression
coverage to base-notation numeric aliases (`"0x1"`, `"0b1"`) in installer/protocol paths, locking
exact numeric strict alias boundary (`"1"` only) behavior; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-323`).
