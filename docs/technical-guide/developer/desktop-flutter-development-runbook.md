...
Latest auth evidence lock: `Unit WS-D-272` optimizes compact normalization delimiter-loop behavior
in `_compactBackendCodeFromTrimmed(...)` by reusing a loop-local output buffer across suffix scans
(`outputBuffer`) to avoid repeated null-coalescing append-buffer assignments on hot iterations
while preserving normalization semantics, and locks leading-delimiter uppercase
`code: "::UNAUTHORIZED"` deterministic auth-required fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-272`).
