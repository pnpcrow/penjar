...
Latest release pipeline evidence lock: `Unit WS-D-315` extends strict parser non-overmatch
regression coverage to near-match aliases (`truee`, `yesplease`, `strict-mode`) in
installer/protocol paths, locking exact-match strict inference boundaries; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-315`).
