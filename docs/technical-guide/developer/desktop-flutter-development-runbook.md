...
Latest release pipeline evidence lock: `Unit WS-D-314` extends strict parser non-overmatch
regression coverage to whitespace-wrapped numeric zero aliases (`" 0 "`) in installer/protocol
paths, locking trim normalization so zero-valued toggles remain non-strict; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-314`).
