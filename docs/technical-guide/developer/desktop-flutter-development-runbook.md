...
Latest release pipeline evidence lock: `Unit WS-D-330` extends strict numeric near-match regression
coverage to control-separator numeric aliases (`"1\\r0"`, `"1\\t0"`) in installer/protocol paths,
locking exact numeric strict alias boundary (`"1"` only) behavior; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-330`).
