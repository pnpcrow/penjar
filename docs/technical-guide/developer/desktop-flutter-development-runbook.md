...
Latest release pipeline evidence lock: `Unit WS-D-297` expands strict/strict-protocol alias matrix
completeness across release/debug paths (including `yes`/`strict`/`true` combinations with strict
failure/warning/pass interactions), reducing alias-path regression gaps; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-297`).
