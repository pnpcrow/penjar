...
Latest auth evidence lock: `Unit WS-D-281` optimizes exact-length candidate gating in
`_classifyBackendCode(...)` by probing raw/compact exact maps only for known marker lengths via
precomputed marker-length catalog reuse, and locks capitalized compact signed-out length-gap suffix
`code: "SignedOutErr"` deterministic auth-required fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-281`).
