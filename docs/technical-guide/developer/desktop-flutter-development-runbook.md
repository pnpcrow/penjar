...
Latest auth evidence lock: `Unit WS-D-229` completed refresh-token direct wrapper signed-out
alias symmetry (`signedOut` / `signed_out` / `isSignedOut` / `loggedOut` / `logged_out`) across
contract+parity suites; see `web-mcp-phase-c-execution-log.md`.
