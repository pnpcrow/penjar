...
Latest auth evidence lock: `Unit WS-D-253` adds an already-compact lowercase fast path in
`_compactBackendCodeFromTrimmed(...)` to skip `toLowerCase` preprocessing when backend codes are
already compact, and locks compact `refreshtokenexpired` fallback behavior in contract/parity
suites while preserving numeric/exact/substring fallback semantics; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-253`).
