...
Latest release pipeline evidence lock: `Unit WS-D-331` extends strict numeric near-match regression
coverage to expression-style numeric aliases (`"1+0"`, `"1-0"`) in installer/protocol paths,
locking exact numeric strict alias boundary (`"1"` only) behavior; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-331`).
