...
Latest release pipeline evidence lock: `Unit WS-D-312` extends strict toggle whitespace regression
coverage to CRLF-wrapped aliases (for example `"\r\nYES\r\n"`, `"\r\nTRUE\r\n"`), locking mixed
Windows line-ending boundary strict semantics in installer/protocol paths; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-312`).
