...
Latest auth evidence lock: `Unit WS-D-248` enforces session-expired marker inclusion inside the
signed-out marker catalog by construction (closing drift for variants like
`ACCESS_TOKEN_EXPIRED`/`REFRESH_TOKEN_EXPIRED`) and adds contract/parity regression locks for
deterministic session-expired fallback plus explicit signed-in override precedence; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-248`).
