...
Latest release pipeline evidence lock: `Unit WS-D-298` improves Windows installer pipeline runtime
efficiency by caching runner-directory existence state (`runner_exists`) across installer/protocol
decision paths, with protocol-stage revalidation to prevent stale-state regressions if installer
commands mutate runner artifacts; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-298`).
