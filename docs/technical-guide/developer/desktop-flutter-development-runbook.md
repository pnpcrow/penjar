...
Latest auth evidence lock: `Unit WS-D-278` optimizes short-code classifier gating in
`_classifyBackendCode(...)` by early-returning before raw-exact lookup/compact normalization when
`trimmed.length` is below signed-out marker minimum length, and locks capitalized compact near-miss
`code: "SignOut"` signed-in/status stability behavior (no unintended auth-required fallback) in
