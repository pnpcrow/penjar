...
Latest release pipeline evidence lock: `Unit WS-D-307` extends strict toggle trim regression
coverage to form-feed/vertical-tab wrapped aliases (for example `"\fYES\f"`, `"\vTRUE\v"`),
locking remaining whitespace-class boundary strict semantics in installer/protocol paths; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-307`).
