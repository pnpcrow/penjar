...
Latest release pipeline evidence lock: `Unit WS-D-288` expands Windows installer pipeline contract
coverage with debug build-mode runner resolution locks, including strict installer pass behavior on
`build/windows/x64/runner/Debug` and strict protocol missing-command failure behavior in debug
mode; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-288`).
