...
Latest auth evidence lock: `Unit WS-D-241` completed parity-side sign-in direct `authState`
matrix guard deduplication across signed-in/signed-in-false/signed-out paths (shared assertion
helper), with execution evidence in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-241`).
