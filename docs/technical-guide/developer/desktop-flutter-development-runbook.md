...
Latest release pipeline evidence lock: `Unit WS-D-302` extends strict toggle trim regression
coverage to tab-wrapped aliases (for example `"\tYES\t"`, `"\tTRUE\t"`) so whitespace-class
normalization remains deterministic for strict installer/protocol parsing; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-302`).
