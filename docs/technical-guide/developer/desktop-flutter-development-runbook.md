...
Latest release pipeline evidence lock: `Unit WS-D-285` adds
`check_windows_installer_pipeline_contract.sh` and wires it into `verify_desktop.sh`, locking
strict Windows protocol-registration command behavior (missing/placeholder fail, clear-command
pass, invalid-mode fail) with deterministic report/log assertions; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-285`).
