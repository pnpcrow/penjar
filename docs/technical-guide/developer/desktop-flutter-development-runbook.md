...
Latest auth evidence lock: `Unit WS-D-280` optimizes marker-loop hot paths in
`_classifyBackendCode(...)` by reusing static marker-count constants for
session-expired/signed-out-only index scans, and locks capitalized compact mixed-marker
`code: "UnauthorizedSessionTimeoutContinuation"` deterministic session-expired fallback precedence
behavior in contract/parity suites; execution evidence is recorded in
