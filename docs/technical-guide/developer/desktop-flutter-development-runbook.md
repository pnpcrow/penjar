...
Latest release pipeline evidence lock: `Unit WS-D-299` locks runner-mutation regression coverage in
Windows installer pipeline contract checks so strict protocol mode keeps protocol-stage behavior
stable (simulated/non-blocking) when installer commands remove runner artifacts before protocol
evaluation across release/debug paths; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-299`).
