...
Latest auth evidence lock: `Unit WS-D-260` expands raw exact-code fast-path classification in
`_classifyBackendCode(...)` to cover both lowercase and uppercase compact exact markers before
compact normalization, and locks lowercase compact `sessionexpired` session-expired fallback
