...
Latest auth evidence lock: `Unit WS-D-273` optimizes compact normalization boundary handling in
`_compactBackendCodeFromTrimmed(...)` by caching `codeLength` across scan/suffix loops and
threading explicit `endExclusive` into compact-uppercase normalization, and locks capitalized
compact `code: "Unauthorized"` deterministic auth-required fallback behavior in contract/parity
suites; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-273`).
