...
Latest auth evidence lock: `Unit WS-D-235` completed sign-in direct-wrapper signed-out alias
symmetry (`signedOut` / `signed_out` / `isSignedOut` / `loggedOut` / `logged_out` /
`isLoggedOut` / `is_signed_out` / `is_logged_out`) across contract+parity suites; see
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-235`).
