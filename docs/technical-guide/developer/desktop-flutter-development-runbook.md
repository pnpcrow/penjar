...
Latest auth evidence lock: `Unit WS-D-242` completed cross-operation parity `authState` matrix
assertion helper consolidation (shared append/expect helpers reused by sign-in, refresh-token,
restore-session, and restore/refresh combined guards), with execution evidence in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-242`).
