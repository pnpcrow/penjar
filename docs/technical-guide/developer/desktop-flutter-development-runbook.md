...
Latest auth evidence lock: `Unit WS-D-252` replaces regex-based backend code compact normalization
with allocation-light character scanning in `_compactBackendCodeFromTrimmed(...)` and locks
delimited `REFRESH-TOKEN::EXPIRED` fallback behavior in contract/parity suites, preserving
numeric/exact/substring fallback semantics; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-252`).
