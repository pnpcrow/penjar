...
Latest auth evidence lock: `Unit WS-D-263` optimizes Unicode-fallback branching in
`_compactBackendCodeFromTrimmed(...)` by short-circuiting immediately when the first non-compact
