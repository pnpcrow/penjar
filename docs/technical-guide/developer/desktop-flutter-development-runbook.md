...
Latest auth evidence lock: `Unit WS-D-270` optimizes compact normalization memory behavior in
`_compactBackendCodeFromTrimmed(...)` by lazily allocating `StringBuffer` only when compact output
append actually occurs, and locks delimiter-only `code: "::"` signed-in/status stability behavior
without unintended auth-required/session-expired fallback mapping in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-270`).
