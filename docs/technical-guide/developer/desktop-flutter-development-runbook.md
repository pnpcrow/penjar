...
1. OS-backed auth snapshot store wiring for remote-stub mode (replace in-memory/noop auth-state store seam with desktop-secure persistence path and restart hydration contract).
