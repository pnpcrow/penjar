...
Latest release pipeline evidence lock: `Unit WS-D-313` optimizes strict toggle normalization in
`run_windows_installer_pipeline.sh` by removing command-substitution from the trim path
(shared-buffer handoff), preserving parser behavior with reduced shell overhead; execution evidence
is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-313`).
