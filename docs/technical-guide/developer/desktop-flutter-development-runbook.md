...
Latest release pipeline evidence lock: `Unit WS-D-308` hardens contract-report Markdown row
stability by escaping control-character/pipe/backslash cell content in
`check_windows_installer_pipeline_contract.sh`, preserving readable mismatch diagnostics for
non-printable strict alias fixtures; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-308`).
