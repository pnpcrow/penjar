...
2. Production Windows protocol-registration command provisioning and signed installer wiring validation (command-hook + strict protocol gate baseline are complete; real registry/installer command secrets and release execution evidence remain pending).
