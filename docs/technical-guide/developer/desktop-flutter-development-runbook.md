...
Latest release pipeline evidence lock: `Unit WS-D-320` extends strict near-match regression
coverage to newline-wrapped uppercase near-match aliases (`"\nTRUEE\n"`) in installer/protocol
paths, locking trim+case boundary non-overmatch behavior; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-320`).
