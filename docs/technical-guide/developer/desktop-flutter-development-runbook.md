...
Latest release pipeline evidence lock: `Unit WS-D-316` hardens
`check_windows_installer_pipeline_contract.sh` with pipeline-script preflight guards (exists +
executable), failing fast with explicit diagnostics before matrix execution when local script state
is invalid; execution evidence is recorded in `web-mcp-phase-c-execution-log.md`
(`Unit WS-D-316`).
