...
Latest auth evidence lock: `Unit WS-D-266` optimizes delimiter-path lowercase prefix append in
`_compactBackendCodeFromTrimmed(...)` by replacing intermediate prefix `substring` allocation with
`_appendCompactBackendAsciiRange(...)` when no uppercase compact units are present, and locks
lowercase delimited `unauthorized::token` auth-required fallback behavior in contract/parity
suites; execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-266`).
