...
Latest release pipeline evidence lock: `Unit WS-D-309` optimizes contract-report markdown cell
escaping by removing per-cell command-substitution subshells in
`check_windows_installer_pipeline_contract.sh`, preserving escaped-output parity with lower shell
overhead; execution evidence is recorded in `web-mcp-phase-c-execution-log.md`
(`Unit WS-D-309`).
