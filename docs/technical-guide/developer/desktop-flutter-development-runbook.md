...
Latest auth evidence lock: `Unit WS-D-282` optimizes exact-length candidate gating in
`_classifyBackendCode(...)` by using bitmask-based marker-length membership checks for raw/compact
exact-map probes, and locks capitalized compact JWT-expired suffix
`code: "JwtExpiredErr"` deterministic session-expired fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-282`).
