...
Latest release pipeline evidence lock: `Unit WS-D-306` extends strict toggle trim regression
coverage to carriage-return-wrapped aliases (for example `"\rYES\r"`, `"\rTRUE\r"`), locking
Windows-style CR boundary strict semantics in installer/protocol paths; execution evidence is
recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-306`).
