...
Latest auth evidence lock: `Unit WS-D-246` applies backend-code classification fast paths for
numeric auth status codes (`401`/`403`/`419`/`440`) and empty compact inputs in
`desktop/lib/contracts/remote_stub_contracts.dart`, preserving signed-out/session-expired semantics
while reducing unnecessary marker-scan work on common status paths; execution evidence is recorded
in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-246`).
