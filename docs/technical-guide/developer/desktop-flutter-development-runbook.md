...
Latest auth evidence lock: `Unit WS-D-276` optimizes marker-loop list access in
`_classifyBackendCode(...)` by caching marker lists as local references before index traversal to
reduce repeated list lookups on classifier hot paths, and locks capitalized compact
`code: "AuthRequired"` deterministic auth-required fallback behavior in contract/parity suites;
execution evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-276`).
