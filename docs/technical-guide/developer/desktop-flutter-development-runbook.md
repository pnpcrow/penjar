...
Latest auth evidence lock: `Unit WS-D-262` optimizes compact uppercase normalization in
`_compactBackendCodeFromTrimmed(...)` through ASCII code-unit lowering
(`_compactBackendAsciiLowercase(...)`) for all-compact mixed-case inputs, and locks mixed-case
compact `SessionExpired` session-expired fallback behavior in contract/parity suites; execution
evidence is recorded in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-262`).
