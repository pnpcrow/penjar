...
Latest auth evidence lock: `Unit WS-D-240` completed parity-side restore-session/refresh-token
direct `authState` matrix guard deduplication (shared assertion helper + payload-filter reuse),
with execution evidence in `web-mcp-phase-c-execution-log.md` (`Unit WS-D-240`).
