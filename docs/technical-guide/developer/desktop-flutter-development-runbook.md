...
Latest release pipeline evidence lock: `Unit WS-D-311` extends strict toggle parser contract
coverage with non-strict guard aliases (`false`, `off`, including whitespace-wrapped ` false `),
locking no-overmatch behavior for installer/protocol strict-mode inference when commands are
missing; execution evidence is recorded in `web-mcp-phase-c-execution-log.md`
(`Unit WS-D-311`).
