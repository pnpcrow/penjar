...
Latest auth evidence lock: `Unit WS-D-269` optimizes delimiter-path uppercase-prefix
normalization in `_compactBackendCodeFromTrimmed(...)` by reusing
`firstUppercaseCompactIndex` and lowercasing only `firstUppercaseCompactIndex..firstNonCompactIndex`
while preserving known-lowercase prefix segments, and locks lowercase-prefix
uppercase-before-delimiter `unauthoriZed::token` auth-required fallback behavior in
contract/parity suites; execution evidence is recorded in
`web-mcp-phase-c-execution-log.md` (`Unit WS-D-269`).
