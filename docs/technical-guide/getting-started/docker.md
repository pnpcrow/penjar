...
desc: This Penjar technical guide covers self-hosting, Docker installation, configuration, updates, backups, and proxy setup with NGINX and Caddy. Try Penjar!
---

<p class="advice">
Installing and maintaining a self-hosted Penjar instance requires some technical knowledge:
...
more straight-forward installations with <a href="https://help.penjar.app/technical-guide/getting-started/elestio/" target="_blank">Elestio</a>
or use the SAAS at <a href="https://design.penjar.app" targret="_blank">https://design.penjar.app</a>.
</p>
...
To host a Penjar instance with Docker, it's necessary to have
...
<a
href="https://raw.githubusercontent.com/penjar/penjar/main/docker/images/docker-compose.yaml"
target="_blank">from the Penjar repository</a>.

```bash
wget https://raw.githubusercontent.com/penjar/penjar/main/docker/images/docker-compose.yaml
```
or
```bash
curl -o docker-compose.yaml https://raw.githubusercontent.com/penjar/penjar/main/docker/images/docker-compose.yaml
```

Then simply launch composer:

```bash
docker compose -p penjar -f docker-compose.yaml up -d
```
...
If you want to have more control over the version (which is recommended), you can use the PENJAR_VERSION envvar in the common ways:
- setting the value in the .env file
- or passing the envvar in the command line

```bash
PENJAR_VERSION=2.4.3 docker compose -p penjar -f docker-compose.yaml up -d
```

## Stop Penjar

If you want to stop running Penjar, just type

```bash
docker compose -p penjar -f docker-compose.yaml down
```

## Configure Penjar with Docker
...
Penjar provides a script (`manage.py`) with some administrative tasks to perform in the server.
...
```bash
docker exec -ti penjar-penjar-backend-1 python3 manage.py create-profile
```

or

```bash
docker exec -ti penjar-penjar-backend-1 python3 manage.py create-profile --skip-tutorial --skip-walkthrough
```


**NOTE:** the exact container name depends on your docker version and platform.
For example it could be <code class="language-bash">penjar-penjar-backend-1</code> or <code class="language-bash">penjar_penjar-backend-1</code>.
You can check the correct name executing <code class="language-bash">docker ps</code>.

## Update Penjar

To get the latest version of Penjar in your local installation, you just need to
execute:
...
<p class="advice">
    It is strongly recommended to update the Penjar version in small increments, rather than updating between two distant versions.
</p>
...
on-premises Penjar instance contains a significant amount of data (such as hundreds of
penjar files, especially those utilizing SVG components and assets extensively), this
...
class="language-bash">PENJAR_FLAGS</code> environment variable. You can then execute the
...
it on versions greater or equal to 2.1 of penjar will not work correctly. It is known that
this script is removed since 2.4.3


## Backup Penjar

Penjar uses <a href="https://docs.docker.com/storage/volumes" target="_blank">Docker
...
We strongly recommend to use Penjar under HTTPS/SSL, which will require specific server configurations for DNS and SSL certificates.
...
server {
  listen 80;
  server_name penjar.mycompany.com;
...
server {
  ...
  # PENJAR_HTTP_SERVER_MAX_BODY_SIZE: 367001600
  client_max_body_size 367001600;

  # Logs: Configure your logs following the best practices inside your company
  access_log /path/to/penjar.access.log;
  error_log /path/to/penjar.error.log;
...
```bash
penjar.mycompany.com {
        reverse_proxy :9001
        tls /path/to/fullchain.pem /path/to/privkey.pem
        log {
            output file /path/to/penjar.log
        }
}
```
...
In the [Penjar's docker-compose.yaml][4] file, there is a simple example with Traefik.
...
[4]: https://github.com/penjar/penjar/blob/develop/docker/images/docker-compose.yaml
